...
class ReportGeneralInspectionPackagingAction
{
    ...
    public function dispatchFilter($params)
    {
        if (! empty($params)) {
            ...
            if (empty($errorMessages)) {
                $packagingInspection = $this->entityManager->createQueryBuilder()
                    ->select('packagingInspection')
                    ->from(
                        'WWII\Domain\Erp\QualityControl\GeneralInspection\PackagingInspection',
                        'packagingInspection'
                    )
                    ->where('packagingInspection.lokasi = :lokasi')
                ...
                $packagingInspection->andWhere('packagingInspection.tanggalInspeksi = :tanggal')
                    ->setParameter('tanggal', $tanggal->format('Y-m-d'));

                $data = $packagingInspection
                    ->getQuery()
                    ->setFirstResult(0)
                    ->setMaxResults(1)
                    ->getOneOrNullResult();
    ...
    protected function render(array $result = null)
    {
        ...
        include('view/report_general_inspection_packaging.phtml');
