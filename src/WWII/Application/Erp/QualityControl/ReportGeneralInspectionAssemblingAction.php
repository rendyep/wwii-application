...
class ReportGeneralInspectionAssemblingAction
{
        ...
        if (! empty($params)) {
            ...
            if (empty($errorMessages)) {
                $assemblingInspection = $this->entityManager->createQueryBuilder()
                    ...
                    ->where('assemblingInspection.lokasi = :lokasi')
                ...
                $assemblingInspection->andWhere('assemblingInspection.tanggalInspeksi = :tanggal')
                    ->setParameter('tanggal', $tanggal->format('Y-m-d'));

                $data = $assemblingInspection
                    ->getQuery()
                    ->setFirstResult(0)
                    ->setMaxResults(1)
                    ->getOneOrNullResult();
    ...
    protected function render(array $result = null)
    {
        ...
        include('view/report_general_inspection_assembling.phtml');
