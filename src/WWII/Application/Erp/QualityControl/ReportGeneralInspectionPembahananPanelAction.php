...
class ReportGeneralInspectionPembahananPanelAction
{
    ...
    public function dispatchFilter($params)
    {
        if (! empty($params)) {
            ...
            if (empty($errorMessages)) {
                $pembahananPanelInspection = $this->entityManager->createQueryBuilder()
                    ...
                    ->where('pembahananPanelInspection.lokasi = :lokasi')
                ...
                $pembahananPanelInspection->andWhere('pembahananPanelInspection.tanggalInspeksi = :tanggal')
                    ->setParameter('tanggal', $tanggal->format('Y-m-d'));

                $data = $pembahananPanelInspection
                    ->getQuery()
                    ->setFirstResult(0)
                    ->setMaxResults(1)
                    ->getOneOrNullResult();
    ...
    protected function render(array $result = null)
    {
        ...
        include('view/report_general_inspection_pembahanan_panel.phtml');
