...
class ReportGeneralInspectionPembahananKayuAction
{
    ...
    public function dispatchFilter($params)
    {
        if (! empty($params)) {
            ...
            if (empty($errorMessages)) {
                $pembahananKayuInspection = $this->entityManager->createQueryBuilder()
                    ...
                    ->where('pembahananKayuInspection.lokasi = :lokasi')
                ...
                $pembahananKayuInspection->andWhere('pembahananKayuInspection.tanggalInspeksi = :tanggal')
                    ->setParameter('tanggal', $tanggal->format('Y-m-d'));

                $data = $pembahananKayuInspection
                    ->getQuery()
                    ->setFirstResult(0)
                    ->setMaxResults(1)
                    ->getOneOrNullResult();
