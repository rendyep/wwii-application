...
class ReportGeneralInspectionWhitewoodAction
{
        ...
        if (! empty($params)) {
            ...
            if (empty($errorMessages)) {
                $whitewoodInspection = $this->entityManager->createQueryBuilder()
                    ...
                    ->where('whitewoodInspection.lokasi = :lokasi')
                ...
                $whitewoodInspection->andWhere('whitewoodInspection.tanggalInspeksi = :tanggal')
                    ->setParameter('tanggal', $tanggal->format('Y-m-d'));

                $data = $whitewoodInspection
                    ->getQuery()
                    ->setFirstResult(0)
                    ->setMaxResults(1)
                    ->getOneOrNullResult();
    ...
    protected function render(array $result = null)
    {
        ...
        include('view/report_general_inspection_whitewood.phtml');
